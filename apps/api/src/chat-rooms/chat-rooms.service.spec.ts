...
describe('ChatRoomsService', () => {
  ...
  describe('find()', () => {
    ...
    it('❌ Should throw an exception when a service error occurs', async () => {
      chatRoomModel.aggregate.mockRejectedValueOnce(
      ...
      await expect(service.find(dto)).rejects.toThrow(
        new InternalServerErrorException('Failed to fetch chat rooms'),
      );
    });
  });
  ...
  describe('findChats()', () => {
    ...
    it('❌ Should throw an exception when a service error occurs', async () => {
      ...
      const dto: FindChatsDto = {
        roomId: '6603f9bcb2d5a5f8c8a6f2e3',
        lastCheckTime: new Date(),
      };

      await expect(service.findChats(dto)).rejects.toThrow(
        new InternalServerErrorException('Failed to fetch chats for room.'),
      );
    });
  });
});
