...
export class GrpcService implements OnModuleInit {
  ...
  onModuleInit() {
    try {
      this.FILE_EXPIRE_WEEK = +this.configService.get<string>('FILE_EXPIRE_WEEK', '4');
      this.IMAGE_EXPIRE_WEEK = +this.configService.get<string>('IMAGE_EXPIRE_WEEK', '2');

      if (isNaN(this.FILE_EXPIRE_WEEK) || isNaN(this.IMAGE_EXPIRE_WEEK)) {
        throw new InternalServerErrorException('Invalid expire week configuration.');
      }
  ...
  runServer() {
    try {
      ...
      if (!fs.existsSync(PROTO_PATH)) {
        throw new InternalServerErrorException(`Proto file not found at ${PROTO_PATH}`);
      }
      ...
      server.bindAsync(
        ...
        (err, port) => {
          if (err) {
            ...
            throw new InternalServerErrorException('Failed to start gRPC server.');
          }
  ...
  uploadFile(call: { request: CreateFileDto }, callback: any) {
    ...
    try {
      ...
      const roomPath = join(__dirname, '../../..', 'bucket', bufferType, roomId);
      ...
      fs.writeFile(filePath, buffer, async (err) => {
        ...
        try {
          const expireAt = moment()
            .add(bufferType === 'file' ? this.FILE_EXPIRE_WEEK : this.IMAGE_EXPIRE_WEEK, 'weeks')
            .startOf('hour');
